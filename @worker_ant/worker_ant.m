classdef worker_ant < ant          %declares worker ant object as a subclass of an ant
    ...
    methods                         %all additional member functions associated with this class are included as separate mfiles in the @ant folder. 
        ...
        function a=worker_ant(varargin) %constructor method for ant  - assigns values to ant properties
                % f=ant(age,food,pos....)
                %age; %Age 
            ...
            switch nargin                     %Use switch statement with nargin,varargin contructs to overload constructor methods
                case 0                        %create default object
                    ...
                    a.w = []; % 
                    a.direction = []; % general direction quadrant
                    
                case 1                         %input is already an ant, so just return!
                    if (isa(varargin{1},'worker_ant'))		
                        a=varargin{1};
                    else
                        error('Input argument is not an worker ant')
                    end
                case 14                         %create a new fox (currently the only constructor method used)
